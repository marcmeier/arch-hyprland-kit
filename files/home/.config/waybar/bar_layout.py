"""Waybar layout shared by density-watch.py (render) and widgets.py (widget manager).

layout.json holds what the widget manager changed. It lives in ~/.local/state/waybar, per machine:
snapshot.sh never takes it into the kit, so every machine keeps its own bar.
