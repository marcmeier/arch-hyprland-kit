...
import datetime as dt, glob, html, json, os, sys, time, urllib.request
...
def api():
    try:
        ...
        req = urllib.request.Request("https://api.anthropic.com/api/oauth/usage",
              headers={"Authorization": "Bearer " + tok, "anthropic-beta": "oauth-2025-04-20", "User-Agent": "claude-code/2"})
...
def limit_block(name, x, p, now):
    ...
    when = (f"resets {r.astimezone():%a %H:%M}  ·  in {fmt_delta((r - now).total_seconds())}" if r and r > now
            else "no active window" if not r else "reset due")
...
def token_table(days, today):
    ...
    for d, v in rows:
        ...
        out.append(f"{lab:<11}" + "".join(f"{fmt_n(x):>{w}}" for x, w in zip(v, (7, 7, 9, 9))))
    return out
...
def main():
    ...
    if data:
        ...
        text = (f"{ICON}  <span size='small' alpha='60%'>5h</span> {sp}%  "
                f"<span size='small' alpha='60%'>7d</span> {wp}%")
        ...
        if win and win + dt.timedelta(hours=5) > now:
            ...
            lines.append(f"<b>Session</b>  ends about {end.astimezone():%H:%M}  ·  in {fmt_delta((end - now).total_seconds())}")
        lines.append(dim("Usage API unavailable (login expired? start Claude Code once)"))
    lines += ["", f"<b>Tokens on this PC</b>"] + token_table(days, today) + ["", dim("Click: open usage page on claude.ai")]
