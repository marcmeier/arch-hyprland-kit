...
def main():
    ...
    status = dict(kv.split("=", 1) for kv in read("status").splitlines() if "=" in kv)
    ...
    if incoming:
        ...
        new = [row[1:] for row in diff.splitlines() if row.startswith("+") and not row.startswith("+++")]
        if new:
            t.append(
                f"  <span color='#ffb454'>new packages: {e(', '.join(new[:10]))}"
                + (" …" if len(new) > 10 else "")
                + "</span>"
            )
    if outgoing:
    ...
    if conflict:
        t += [
            "",
            f"<span color='#ff6b6b'><b>Changed here and on GitHub:</b></span> {e(', '.join(conflict))}",
    ...
    if untrusted:
        ...
        if hosts:
            t.append(
                f"<span color='#ffb454'><b>New machine: {e(', '.join(hosts))}</b></span> (menu: Trust new machine)"
            )
        if foreign:
            t.append(
    ...
    for line in git("log", "-300", "--format=%ct%x1f%s", "origin/main" if has_origin else "HEAD").splitlines():
        ...
        if m and m.group(1) not in seen:
            seen[m.group(1)] = int(ct)
    if seen:
        t += ["", "Last change per machine:"] + [
            f"  {e(h)}: {ago(ts)}" for h, ts in sorted(seen.items(), key=lambda x: -x[1])
        ]
