...
def emit():
    ...
    for a in list(urgent):
        if clients.get(a) in (None, active): urgent.discard(a)
    if MORE:
        extra = sorted(i for i, n in wins.items() if i > LAST and n)
        if active and active > LAST and active not in extra: extra.append(active)
        extra.sort()
        if not extra:
            print(json.dumps({"text": ""}), flush=True); return
        cls = ["ws", "more"]
        if active and active > LAST: cls.append("active")
        if any(clients.get(a, 0) > LAST for a in urgent): cls.append("urgent")
        ...
        return
    cls = ["ws"]
    if active == N: cls.append("active")
    if any(clients.get(a) == N for a in urgent): cls.append("urgent")
    if not wins.get(N): cls.append("empty")
    print(json.dumps({"text": str(N), "class": cls, "tooltip": ""}), flush=True)  # False is shown as the text "false"

sock_path = os.environ.get("WS_SOCKET") or f"{os.environ['XDG_RUNTIME_DIR']}/hypr/{os.environ['HYPRLAND_INSTANCE_SIGNATURE']}/.socket2.sock"
emit()
...
while True:
    ...
    *lines, buf = buf.split(b"\n")
    dirty = False
    for l in lines:
        ev, _, arg = l.decode(errors="replace").partition(">>")
        if ev == "urgent":
            urgent.add(arg.strip().removeprefix("0x")); dirty = True
        elif ev in ("workspacev2", "workspace", "focusedmon", "activewindowv2", "openwindow",
                    "closewindow", "movewindowv2", "createworkspacev2", "destroyworkspacev2"):
            dirty = True
    if dirty:
        emit()
