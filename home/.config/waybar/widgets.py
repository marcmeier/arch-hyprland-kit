...
Changes go to ~/.local/state/waybar/layout.json, per machine (bar_layout.py); after each change the
bar restarts (render.py applies the layout when waybar starts).
...
def save_layout(layout):
    _save_layout(layout)
    subprocess.run(["systemctl", "--user", "restart", "waybar.service"], check=False)
