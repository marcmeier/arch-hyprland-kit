...
not_owned=$(find "$HOME" -xdev \( ! -user "$USER" \) 2> /dev/null | wc -l)
if ((not_owned == 0)); then ok "everything below \$HOME belongs to $USER"; else
  fail "$not_owned entries below \$HOME not owned by $USER  ->  sudo chown -R $USER:$(id -gn) $HOME"
...
((differ)) && info "$differ files differ from the kit (normal: wallpaper theme, monitor/battery patches, later edits): ${DIFFERING[*]:0:8}"
...
SYNC_STATE=${XDG_STATE_HOME:-$HOME/.local/state}/rebuild
if [[ $(git -C "$KIT" config commit.gpgsign) == true && -f $HOME/.ssh/rebuild-signing && -s $SYNC_STATE/allowed_signers ]] &&
  grep -qF "$(awk '{ print $2 }' "$HOME/.ssh/rebuild-signing.pub")" "$SYNC_STATE/allowed_signers"; then
  ok "kit commits are signed and checked ($(grep -c . "$SYNC_STATE/allowed_signers") trusted machine(s))"
...
[[ -z $failed ]] && ok "no failed system units" || warn "failed system units: $failed"
failed=$(systemctl --user --failed --no-legend 2> /dev/null | awk '{print $2}' | tr '\n' ' ')
[[ -z $failed ]] && ok "no failed user units" || warn "failed user units: $failed"

sec "6. Boot"
BOOT_ENTRIES=/boot/loader/entries
[[ -f $BOOT_ENTRIES/arch.conf ]] && ok "arch.conf exists" || fail "arch.conf missing"
grep -q '\bsplash\b' "$BOOT_ENTRIES/arch.conf" 2> /dev/null && ok "arch.conf has quiet splash" || warn "arch.conf lacks splash"
[[ -f /boot/vmlinuz-linux-lts ]] && { [[ -f $BOOT_ENTRIES/arch-lts.conf ]] && ok "arch-lts.conf exists" || fail "arch-lts.conf missing"; }
...
for entry in "$BOOT_ENTRIES"/*.conf; do
  while read -r initrd; do
    [[ -f /boot$initrd ]] || fail "$(basename "$entry"): initrd /boot$initrd does not exist"
  done < <(awk '$1=="initrd"{print $2}' "$entry")
  kernel=$(awk '$1=="linux"{print $2}' "$entry")
  [[ -f /boot$kernel ]] || fail "$(basename "$entry"): kernel /boot$kernel does not exist"
done
if ((HW_SURFACE)); then
  ...
  [[ -f $BOOT_ENTRIES/arch-surface.conf ]] && ok "arch-surface.conf exists" || fail "arch-surface.conf missing"
...
for entry in "$BOOT_ENTRIES"/*surface*.conf; do [[ -f $entry ]] && { grep -q '\bsplash\b' "$entry" && ok "$(basename "$entry") has splash" || warn "$(basename "$entry") lacks quiet splash"; }; done
...
[[ -n $gtk_theme ]] && ok "GTK theme: $gtk_theme" || warn "gsettings unavailable (no session bus?)"
