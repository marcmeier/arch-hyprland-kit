...
if (( n == 0 )); then ok "everything below \$HOME belongs to $USER"; else
  fail "$n entries below \$HOME not owned by $USER  ->  sudo chown -R $USER:$(id -gn) $HOME"; find "$HOME" -xdev ! -user "$USER" 2>/dev/null | head -5 | sed 's/^/          /'; fi
...
[[ -z $miss ]] && ok "all $(grep -c . "$KIT/packages/pacman.txt") pacman packages installed" || { fail "missing pacman packages: $(echo $miss)"; }
miss=$(comm -23 <(sort -u "$KIT/packages/aur.txt" | grep .) <(pacman -Qqm | sort))
[[ -z $miss ]] && ok "all $(grep -c . "$KIT/packages/aur.txt") AUR packages installed" || fail "missing AUR packages: $(echo $miss)"
miss=""; for p in "${HWP[@]}"; do [[ -n $p ]] && ! pacman -Qq "$p" >/dev/null 2>&1 && miss+="$p "; done
...
missing=0; differ=0; DL=()
while IFS= read -r f; do
  f="${f#./}"
  if [[ ! -e $HOME/$f ]]; then fail "missing: ~/$f"; missing=$((missing+1))
  elif ! cmp -s "$KIT/files/home/$f" "$HOME/$f"; then differ=$((differ+1)); DL+=("$f"); fi
...
for e in $E/*.conf; do
  for i in $(awk '$1=="initrd"{print $2}' "$e"); do [[ -f /boot$i ]] || fail "$(basename "$e"): initrd /boot$i does not exist"; done
  k=$(awk '$1=="linux"{print $2}' "$e"); [[ -f /boot$k ]] || fail "$(basename "$e"): kernel /boot$k does not exist"
done
if (( HW_SURFACE )); then
...
sec "7. User setup"
for d in Projects Games; do [[ -d $HOME/$d ]] && ok "~/$d" || fail "~/$d missing"; done
miss=""; for d in Desktop Downloads Documents Music Pictures Videos Templates Public; do [[ -d $HOME/$d ]] || miss+="$d "; done
[[ -z $miss ]] && ok "standard folders (Desktop, Downloads, Documents, ...)" || fail "missing standard folders: $miss ->  xdg-user-dirs-update"
t=$(gsettings get org.gnome.desktop.interface gtk-theme 2>/dev/null); [[ -n $t ]] && ok "GTK theme: $t" || warn "gsettings unavailable (no session bus?)"
...
[[ -L $HOME/.local/bin/set-wallpaper ]] && ok "set-wallpaper link" || fail "~/.local/bin/set-wallpaper missing"
...
if [[ -n ${KIT_NOTES_REL:-} ]]; then
  [[ -f $HOME/$KIT_NOTES_REL/CLAUDE.md ]] && ok "~/$KIT_NOTES_REL/CLAUDE.md" || warn "~/$KIT_NOTES_REL/CLAUDE.md missing"
fi
